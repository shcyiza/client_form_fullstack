...
const CHARGE_CARD_MTTN = `mutation ($token: String!, $order_id: String!){
    ChargeCard(token: $token, order_id: $order_id) {
        is_paid
        payment_ref
    }
}`;
...
export async function chargeCard(token, order_id) {
    const query = clientFormGraph(CHARGE_CARD_MTTN);
